...
### Icons & Fonts
- **Line Awesome** - Icon library for UI controls
- **Omnes Pro** (Adobe Fonts) - Primary typeface
...
JSON file containing track metadata:
...
2. Extracts ID3 metadata (title, artist, album, duration)
3. Extracts embedded album artwork
4. Saves artwork to `src/img/` with sanitized filenames
5. Generates color palette using Vibrant algorithm
6. Creates `tracks.json` with all metadata and palettes
7. Automatically converts audio file paths to `.opus` extension for optimized delivery
...
3. Configuration:
   ...
   - Channels: 2 (stereo)
   - Sample rate: 48kHz
4. Outputs ONLY `.opus` files to `dist/music/` (MP3s are never copied to dist)
5. Reports file size reduction statistics
...
- `concurrently@^9.2.1` - Parallel task execution
- `cpx@^1.5.0` - File copying with glob support
- `fluent-ffmpeg@^2.1.3` - FFmpeg Node wrapper
- `live-server@^1.2.2` - Development server
- `music-metadata@^11.9.0` - ID3 tag parsing
